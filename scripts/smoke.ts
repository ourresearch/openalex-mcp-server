...
const calls: Array<[string, Record<string, any>, (r: any) => void]> = [
  ...
  // ---- retractions hidden by default, opt-in to include (oxjob #1281) ----
  ["search_works", { oql: "works where title has (hydroxychloroquine) and year >= (2020)", preview: true }, (r) => { if (!/retracted is \(false\)/.test(r.oql)) throw new Error("no default retracted clause: " + r.oql); if (!/excluded by default/.test(r.retracted_works ?? "")) throw new Error("no retracted_works note"); if (r.results.some((w: any) => w.is_retracted)) throw new Error("retracted row leaked"); }],
  ["search_works", { oql: "works where title has (hydroxychloroquine) and year >= (2020)", preview: true, include_retracted: true }, (r) => { if (/retracted/.test(r.oql)) throw new Error("opt-in still filtered: " + r.oql); if (!/included/.test(r.retracted_works ?? "")) throw new Error("no included note"); }],
  ["search_works", { oql: "works where retracted is (true)", preview: true, preview_limit: 5 }, (r) => { if ((r.oql.match(/retracted/g) ?? []).length !== 1) throw new Error("double retracted clause: " + r.oql); if (!r.results.length || !r.results.every((w: any) => w.is_retracted === true)) throw new Error("expected only retracted rows"); }],
  ["search_works", { query: "hydroxychloroquine", from_year: 2020, limit: 3 }, (r) => { if (!/is_retracted:false/.test(r.filter)) throw new Error("structured path lost the default"); if (!r.retracted_works) throw new Error("no retracted_works note"); }],
  ["search_works", { author_ids: ["A5012924666"], for_author: "A5012924666", limit: 3 }, (r) => { if (/is_retracted:false/.test(r.filter)) throw new Error("audit view hid retractions: " + r.filter); }],
  ["get_work", { id: "10.1016/j.ijantimicag.2020.105949" }, (r) => { const k = Object.keys(r); if (k[0] !== "is_retracted" || k[1] !== "warning") throw new Error("retracted work not flagged first: " + k.slice(0, 3).join(",")); }],
  ["resolve_references", { references: ["10.1016/j.ijantimicag.2020.105949", "10.7717/peerj.4375"] }, (r) => { if (r.summary.retracted !== 1) throw new Error("retracted count " + r.summary.retracted); if (!r.results[0].retracted || !r.results[0].retraction_warning || !r.note) throw new Error("retracted item not flagged"); if (r.results[1].retracted) throw new Error("clean item flagged"); }],
  ["group_works", { oql: "works where title has (hydroxychloroquine)", group_by: "year", limit: 5 }, (r) => { if (!/retracted is \(false\)/.test(r.oql)) throw new Error("group_works oql lacks retracted clause: " + r.oql); if (!r.retracted_works) throw new Error("no note"); }],
  ["list_citations", { work_id: "W2741809807", limit: 3 }, (r) => { if (!/excluded by default/.test(r.retracted_works ?? "")) throw new Error("no retracted_works note"); }],
  ["analyze_works", { institution_ids: ["I27837315"], from_year: 2023, sections: ["by_type"] }, (r) => { if (!/excluded by default/.test(r.retracted_works ?? "")) throw new Error("no retracted_works note"); }],
];
